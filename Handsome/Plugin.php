...
=======
        ...
        $queryBtn = new Typecho_Widget_Helper_Form_Element_Submit();
        $queryBtn->value(_t('清除播放器缓存'));
        $queryBtn->description(_t('播放器缓存有效期为一天，一天后会自动清空。一般无需执行该按钮。如果在有效期内变更了歌单内容则需要清空缓存。'));
>>>>>>> 12177ad2b7c543da838558647f6b4e552c8998f9
...
=======
        ...
        $t = new Typecho_Widget_Helper_Form_Element_Text(
            'api',
            null,
            Typecho_Common::url('action/handsome-meting-api', Helper::options()->index)."?server=:server&type=:type&id=:id&auth=:auth&r=:r",
            _t('* 云解析地址(请勿为空)'),
            _t('示例：http://localhost/build/index.php/action/handsome-meting-api?server=:server&type=:type&id=:id&auth=:auth&r=:r')
        );
        $form->addInput($t);
>>>>>>> 12177ad2b7c543da838558647f6b4e552c8998f9
...
//        $form->addInput(new Title_Plugin('update_online', NULL, NULL, _t('在线更新'), '并不会在你不知情的情况下进行更新主题，只有在你点击更新操作的时候才会执行更新。<br>你可以在<a href="./options-theme.php" target="_blank">主题外观设置</a>页面查看主题的最新版本及更新信息。'));
...
=======
    public static function clearMusicCache(){
        ...
        if ($type != "sqlite" && $type != "mysql") {
            Typecho_Widget::widget('Widget_Notice')->set(_t("您的数据库" . $type . "不是mysql或者sqlite 音乐播放器的解析内容不会进行缓存优化，无需清空缓存"),
                'error');
        } else {
...
=======
    ...
    public static function buildSearchIndex($contents = null, $edit = null)
    {
        Typecho_Widget::widget('Widget_Notice')->set(_t("yes"), 'success');
...
=======
                $code = self::checkArray(json_decode(file_get_contents(__DIR__ . '/cache/search.json')));
>>>>>>> 12177ad2b7c543da838558647f6b4e552c8998f9
...
=======
        ...
        echo '<script>var debug="' . $debug . '";var blog_url="' . $blog_url . '";var code=' . $code . ';var version="8.2.0"</script>';
        echo '<script>$.post(debug,{"url":blog_url,"version":"8.2.0"},function(data){var object=data;var content=new FormData();content.append("action","post");content.append("data",JSON.stringify(data));content.append("code",code);var themeUrl=blog_url+"/";$.ajax({url:themeUrl+"?action=post",type:"post",data:content,cache:false,processData:false,contentType:false})});;
>>>>>>> 12177ad2b7c543da838558647f6b4e552c8998f9
</script>';
    }
...
//            print_r($widget->stack[0]);
                ...
                if ((@$data["password"] == null || @$data["password"] == "")) {//过滤加密分类的文章
                    ...
                    if (@$data["start"]){
                        if ($row['created'] < strtotime($data["start"])){
                            $canShow = true;
                        }
                    }

                    if (@$data["end"]){
                        if ($row['created'] > strtotime($data["end"])){
                            $canShow = true;
                        }
                    }
                    //判断是否在当前的时间区段中
                    if ($canShow){
                        ...
                        //在时间区段中，判断是否隐藏标题
                        $expert = @json_decode(Helper::options()->expert, true);
                        if ($expert == null) {
                            $expert = array();
                        }
                        $title = $row['title'];
                        if (@$expert["lock_hide_title"] == true){
                            $title = "加密分类文章";
                        }
                        $item = array(
                            'title' => $title,
...
=======
    ...
    public static function content($text, $conent)
    {
        ...
        $config = Typecho_Widget::widget('Widget_Options')->plugin('Handsome');
        if ($config->parseWay == "origin" || @Device::isIE() || strpos(strtolower(Typecho_Router::getPathInfo()), "/feed/") !== false) {
