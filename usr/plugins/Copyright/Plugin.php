...
class Copyright_Plugin implements Typecho_Plugin_Interface {
    ...
    private static function render($cr) {
        ...
        if ($cr['is_enable']) {
            if ($cr['author']) {
                $t_author = '<p class="content-copyright">版权声明：' . $cr['author'] . '</p>';
            }
            if ($cr['url']) {
                if ($cr['is_original']) {
            ...
            $copyright_html = '<blockquote class="content-copyright" style="margin-top:50px;padding:16px;background-color:rgba(255,255,255,0.7);border-left:5px solid rgba(114,102,186,0.7)!important;">' . $t_author . $t_url . $t_notice . '</blockquote>';
        }
